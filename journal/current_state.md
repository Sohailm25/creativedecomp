...
- Focus: keep the `resattn` operating structure while removing sibling-project assumptions from the core docs and locking this repo to the creativity experiment
- Experimental status: the workspace now has the required operating files, result lanes, history docs, beads tracking, and a green local test suite, but there are no model runs yet
